...
void QNanoPainter::deleteImage(const QString& uniqueKey)
{
    if(m_dataCache.contains(uniqueKey)){
        auto ref = m_dataCache[uniqueKey]->ref;
        if(ref<=1){
            auto e = m_dataCache.take(uniqueKey);
            nvgDeleteImage(m_nvgContext, e->id);
            qDebug() << "<<<--->>>" << uniqueKey;
        }else{
            --m_dataCache[uniqueKey]->ref;
        }
    }
}
